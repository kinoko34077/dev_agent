# Status: ARCHIVED
# Superseded by: `docs/V2_EXECUTION_PLAN.md` and `docs/V2_DETAILED_ROADMAP.md`
#
# dev_agent v2 実行計画
...
| 軸 | 内容 |
| --- | --- |
| **Implementation Frontier**（実装が到達している範囲） | Phase 7 A〜Eのdeterministic evaluator、explicit review、EscalationExecutor、Evaluator→dispatch cycle、TaskLifecycleCoordinator、有限Lifecycle合成、workflow proposal境界、Root Planning、proposal-only `ModelPlanningAdapter`（注入Provider＋厳格JSON decode＋明示exact-tier routing）とHost-only `DevelopmentPlanningBridge`（validated Commander Plan候補を返すが`.devfarm`へ直接書かない）、AgentBackend thin contract + dispatcher、development Commander、Phase 7後半A: `CodexExecBackend`（`codex exec`をbounded non-blocking subprocessとして実行する最初のconcrete adapter。exit codeのみを正とし、subprocess自身の自己申告を信用しない。temporary HOMEはreversible quarantine、認証は明示されたauth.json一枚のみ投影）と、development-only DevFarm bridge（untrackedを含むHost側Git差分検証、contained Host Verification、auto-integrationなし）、Group D初回sliceのCodex JSONL構造化イベント／usage正規化、SupervisorのCodex action return境界、開発専用固定Worker capability probe梯子、実Free WorkerのHost Verification／durable review／deterministic integration／REWORK／依存baseline更新まで実装・dogfood済み。続くGroup D境界としてlocal `client_session_key`とProvider `external_session_id`の契約、bounded `artifact_references`のJSON-safe durable projection、およびWorker境界例外のper-attempt durable failure evidenceを追加済み。ただしCodex session discovery/restartは未着手・未実証。実Free L2 Plannerのlive shadow dogfood、MCP Adapter、OS Sandbox、Evidence routing接続、Workflow Promotionも未着手 |
...
| **External Blockers**（コード変更では閉じられない外部条件） | G6O1（`spec/v2/G6O1_DEFERRED.md`により未検証のまま凍結・現行roadmapは非ブロッキング。実paid Providerのworst-case課金実証＋deployment-owned budget設定は将来再開時の外部条件）、OS_SANDBOXED（filesystem/network/process/resource isolationの実証）。GitHub branch protectionはrequired status checksを設定済みだが、明示設定されたユーザーbypassによる直接push時はCIをpush後に確認する運用である。 |
| **Next Development Target**（次に着手する開発） | 日常Supervisor導線と実Free WorkerのHost Verification／durable review／deterministic integration／REWORK／依存baseline更新の既存証拠を維持する。Group Dではartifact identity/reference projectionと、UNKNOWN後のsession/result replay projectionを追加済み。次はrestart/reconciliationの明示的な外部session discovery authorityを、既存AgentBackend contractの範囲で検証する。CodexExecBackendへ安全な自動discoveryを推測実装しない。実Gemini WorkerのGroup D候補は2 bounded attemptがWorker境界例外で失敗しており、成功dogfoodへは未到達。proposal-only Free L2 Planner adapterとHost validation bridgeは追加済みだが、実Planner shadow dogfoodとCommanderへの実運用接続は未実証。外部WorkerのSTATIC_ONLY境界、自動push/mergeなし、Compression Service未接続、G6O1-SIM/LIVE未検証、OS-level sandbox未提供を維持する。 |
