...
- Free L2 Reviewer shadow has two live proposal-only comparison samples and remains non-authoritative. The bounded D7 candidate cycle is live-evidence verified, but official-branch Codex-less integration is not enabled. D8 Host F0–F2 composition is verified, while model-driven diagnosis, automatic improvement dispatch/repair, and official-branch promotion remain unconnected.
...
4. Use the D7 candidate evidence as the bounded baseline; keep official-branch Codex-less integration disabled and extend D8 only through the existing Host proposal boundary before considering D9 candidate contracts.
