...
| --- | --- |
| Branch | `v2/bootstrap` |
| Current HEAD | `caf52ad` (`feat: add credential manager compression composition`) |
| Implementation/evidence baseline | `caf52ad` (Credential Manager composition, safe HTTP edge diagnostics, and the preceding coordination/egress foundation) |
| Worktree | clean at the current verification checkpoint; this documentation sync records the same checkpoint |
| Local regression | `1126 passed, 1 skipped` (`python -m pytest tests/v2 -q`, 252.69s) |
| Architecture | `ARCHITECTURE_PASS` |
| Compile | `python -m compileall -q src recovery scripts` PASS |
| Exact-head CI | PASS for current HEAD `caf52ad5f247b8d84e344b34f05a5b9b52daa55d`: `v2-core` [run 34845670338](https://github.com/kinoko34077/dev_agent/actions/runs/34845670338) and `v2-provider-smoke` [run 34845670401](https://github.com/kinoko34077/dev_agent/actions/runs/34845670401). |
...
- CI regression follow-up: exact-head `a26c33c` passed `kernel (3.10)`, `kernel (3.11)`, and `provider-smoke` after the provider-saturation test fixture was made deterministic; the subsequent documentation checkpoints `39e08d9`, `ec0632d`, and `f072726` also passed all three checks. The fixture change did not alter production dispatch or UNKNOWN/reconciliation semantics.
