...
| Discord Human UI | `src/dev_agent/discord/` | optional discord.py Gateway ingress、bounded renderer、Discord identity/scope、durable channel/thread pointer、Human/Approval/Core adapter | Human / StateStore / Operation / Process Coordination callbacks | no Discord-owned Task/Scheduler/approval authority、token redaction、exact response correlation | `tests/v2/test_discord_adapter.py`, `test_discord_bot.py`, `test_discord_core_adapter.py`, `test_discord_durable_flow.py`, `spec/v2/evidence/discord-human-ui-local-20260923.json` |
