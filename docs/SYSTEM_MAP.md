...
| Discord Human UI | `src/dev_agent/discord/` | optional discord.py Gateway ingress、bounded renderer、Discord identity/scope、durable channel/thread pointer、`DiscordRuntimeComposition`による既存Operation／Process Coordination／Human／Approval境界へのrunner注入 | Human / StateStore / Operation / Process Coordination callbacks | no Discord-owned Task/Scheduler/approval authority、unauthorized ingress silent、token redaction、exact response correlation | `tests/v2/test_discord_adapter.py`, `test_discord_bot.py`, `test_discord_composition.py`, `test_discord_core_adapter.py`, `test_discord_durable_flow.py`, `spec/v2/evidence/discord-gateway-login-20260923.json`, `spec/v2/evidence/discord-core-runner-composition-20260923.json` |
