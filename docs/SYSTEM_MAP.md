...
| Process Coordination | `src/dev_agent/coordination/` | peer identity/generation、presence lease、durable mailbox、immutable handoff artifacts、Work Address/Resume projection、generation-fenced ControlRequest、read-only Guardian evaluation | SQLite primitive / security audit | no Task/Scheduler/process launcher authority; Guardian policy has no process side effect | `tests/v2/test_process_coordination_*.py`, `docs/requirements/process-coordination/`, `docs/CODEX_WORK_COORDINATION.md` |
