...
- [ ] Add failing tests for backend session identity, non-replayed sequence, approval/cancelling status, and artifact reference validation.
- [ ] Run `python -m pytest tests/v2/test_agent_backend_protocol.py -q`; confirm the new assertions fail for missing boundary behavior.
- [ ] Implement only generic validation and status representation; do not add Codex fields.
- [ ] Re-run the focused test and the existing protocol regression.
...
- [ ] Write failing tests for missing task/scope, denied approval, completed dispatch deduplication, and immutable request fingerprint.
- [ ] Run the focused dispatcher tests and confirm they fail because the dispatcher does not exist.
- [ ] Implement validation and durable dispatch identity using existing storage/effect primitives; call only `backend.start()` after checks pass.
- [ ] Add event/result collection methods that validate session identity and preserve UNKNOWN versus RECONCILING.
- [ ] Make COMPLETED idempotent, and reject automatic retry for UNKNOWN/RECONCILING.
- [ ] Run dispatcher focused tests and existing state/effect regressions.
...
- [ ] Write failing tests covering start, ordered events, result, cancel, approval wait, backend death, dispatcher restart, and UNKNOWN→RECONCILING without replay.
- [ ] Run the focused tests to verify the intended failures.
- [ ] Implement a test-only in-memory deterministic fake with explicit scenario configuration; it must not enter production imports or own production state or scheduler behavior.
- [ ] Add restart simulation through reusing the same backend session identifier and a separate dispatcher instance.
- [ ] Run focused tests and all `tests/v2`.
...
- [ ] Record the dispatcher and fake lifecycle boundary without claiming Codex App Server live integration.
- [ ] Record focused and full regression counts from fresh commands.
- [ ] Preserve G6O1 external-blocked status and Evidence routing advisory-only status.
- [ ] Run `git diff --check`, JSON/spec checks, and the full v2 regression.
- [ ] Commit code/tests and documentation separately, then push `v2/bootstrap`.
