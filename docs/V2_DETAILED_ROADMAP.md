...
- 2026-09-17追加: `b29019a`では、Provider応答まで到達したPlannerの`invalid_json` / `invalid_proposal`に限り、Hostが別admissionしたL1 planning Criticへ一回だけ補正を委譲できるようにした。Criticは`corrected_proposal`のみを返し、既存の`RootPlanningValidator`が最終判定する。成功Planner、transport/quota/security/UNKNOWN failureはCriticを呼ばず、同一provider/binding/modelの重複poolも拒否する。CLI接続を含む契約・ローカル検証Evidenceであり、live Planner recovery、AR1、Phase 8 LIVE_ACTIVATIONではない。Evidenceは[`planner-independent-critic-contract-20260917.json`](../spec/v2/evidence/planner-independent-critic-contract-20260917.json)を参照する。
