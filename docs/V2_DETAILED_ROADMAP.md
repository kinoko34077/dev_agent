...
**2026-09-26 Issue #10 state/evidence hygiene:** `docs/CURRENT_STATE.md`
now begins with a compact operational handoff, while historical chronology
remains available through Git history, Issues/PRs, and bounded Evidence.
`spec/v2/GATE_STATUS.json` retains the same machine statuses and now describes
the later completed local L1 root without treating it as formal Phase 8 live
activation.
...
**2026-09-26 Phase 8 production composition checkpoint:** `f47d382` closes the
deterministic local composition chain: one fresh Operation root is submitted,
validated Planner children are handed to the single existing DevFarm owner,
two non-overlapping Workers pass Host Verification and separate Reviewer /
final-decision boundaries, deterministic integration records `CODE_INTEGRATED`,
and one existing RuntimeCoordinator cycle completes the released dependent
continuation. The root is parked before that cycle, its queue item has zero
claims, and the root then reaches `COMPLETED`. The public driver remains only
submit/observe and no scheduler, queue, StateStore, retry engine, or authority
was added. Provider responses are fixtures, so this is non-Gate local
composition evidence; remote generation admission, Discord live E2E, and
...
**2026-09-23 Discord Human UI checkpoint:** the optional `discord.py` adapter is locally composed around existing Human/Approval/Operation/Process Coordination boundaries. Durable state contains only Discord channel/thread pointers, ingress message idempotency, and delivery metadata; Human requests/responses remain in the existing SQLite HumanInteractionPort and consume once by exact `request_id`. The Bot Token is confined to the ignored local `.env`; a fresh Host smoke reached Discord Gateway login, `on_ready`, and application-command synchronization. The Portal setting was not independently reverified in the repository and no interactive client was available for a new message, so ordinary message content and live UI E2E remain pending. This is a non-Gate local UI slice; D9 Production Deployment and Phase 8 LIVE_ACTIVATION remain unchanged. Evidence: [`discord-gateway-login-20260923.json`](../spec/v2/evidence/discord-gateway-login-20260923.json) and [`discord-human-ui-local-20260923.json`](../spec/v2/evidence/discord-human-ui-local-20260923.json).

**2026-09-23 Discord standard-runner composition checkpoint:** `336be54` makes the standalone runner compose the existing SQLite StateStore, Operation submit/status boundary, HumanInteractionPort, proposal-only Approval adapter, and Process Coordination mailbox through `DiscordRuntimeComposition`; it does not create a second scheduler or Task state owner. Rejected/duplicate/bot messages are silent, and accepted read queries use a bounded status projection. This is local non-Gate evidence only; a real Discord client message and live Gateway E2E remain unverified. Evidence: [`discord-core-runner-composition-20260923.json`](../spec/v2/evidence/discord-core-runner-composition-20260923.json).

**2026-09-23 Discord outbound projection checkpoint:** the standard runner starts one read-only `DiscordOutboundPublisher` after Gateway readiness. It observes existing HumanRequest/task/event state, resolves existing channel/thread pointers, projects bounded progress and HumanRequest messages, and uses existing durable delivery metadata to avoid replay on pump re-entry. Transient observation/send errors are isolated to the next bounded pass. Approval buttons remain disabled unless an explicit Core-owned approval view/submit boundary is injected; no approval authority is created in Discord. This is local projection evidence only and does not close live Discord E2E or any formal Gate. Evidence: [`discord-outbound-projection-20260923.json`](../spec/v2/evidence/discord-outbound-projection-20260923.json).

The subsequent R10-PREP tiny-root continuation produced the same bounded result: fresh local Planner validation passed, but the first Worker task only advanced once before a repeated invalid-JSON signature and one failed Gemma fallback. It remains weak-model convergence preparation evidence, not a live Phase 8 root, formal R10 benchmark, or Gate transition. Evidence: [`ollama-local-r10-tiny-worker-20260923.json`](../spec/v2/evidence/ollama-local-r10-tiny-worker-20260923.json).

The next fresh local root exercised the strengthened latest-repair rebind and recurrence path. qwen3.5:9b Planner validation passed; two non-overlapping Worker tasks were attempted, with qwen9 primary and Gemma4 fallback. Both produced Host Verification failures in generated syntax, so Reviewer, deterministic integration, and `CODE_INTEGRATED` continuation were not reached. This remains `OLLAMA_LOCAL_E2E=PARTIAL_BLOCKED` and does not promote Phase 8. Evidence: [`ollama-local-fresh-root-rebind-20260923.json`](../spec/v2/evidence/ollama-local-fresh-root-rebind-20260923.json).

The next local timeout/convergence follow-up fixed a Host configuration propagation gap: the existing bounded `HostProcessExecutor` timeout is now passed to the static Ollama child, so local model load time no longer consumes an unobserved 30-second child adapter default. A fresh qwen9/Gemma two-task observation reached Host Verification for task A but task B remained blocked after concrete Gemma corrections. This remains R10-PREP/local evidence only; Reviewer, integration, dependent continuation, and `PHASE8 LIVE_ACTIVATION` are not complete. Evidence: [`ollama-local-timeout-convergence-20260923.json`](../spec/v2/evidence/ollama-local-timeout-convergence-20260923.json).
...
- 2026-09-17追加: bounded convergence R0–R6のmetadata/fingerprint、Validation Ladder、finite progress assessment、Planner correction、Worker/Reviewer refinementを、R7/R8として既存Operation statusとRuntimeCoordinatorへ薄く接続した。reconciliation待ちのdurable Taskを保持しながら無関係なREADY workを継続し、Phase 8相当のtask graphをrestart後に再適用してもtask/attempt重複がないことをlocal deterministic regressionで確認した。これはlive AR1やPhase 8 LIVE_ACTIVATIONではない。Evidenceは[`bounded-convergence-runtime-r7-r8-20260917.json`](../spec/v2/evidence/bounded-convergence-runtime-r7-r8-20260917.json)。
- 2026-09-17追加: `2cb6639`でPlanner ShadowのProvider呼出し前に閉じるroute/admission/resource-pool組成失敗を`blocked_local` / `reconciliation_required=false`へ分離した。raw exception detailは投影せず、Provider transport taxonomy、failover、quota、UNKNOWN replay、Gateは変更していない。これはR9診断境界の補強であり、live Planner成功やPhase 8 LIVE_ACTIVATIONではない。Evidenceは[`planner-shadow-local-admission-diagnostics-20260917.json`](../spec/v2/evidence/planner-shadow-local-admission-diagnostics-20260917.json)。
- 2026-09-17追加: `416ce96`で既存R3 bounded Planner convergenceをPlanner Shadow CLIへ接続した。独立admission済みL1 Critic poolを設定した場合、valid Planner responseは`FAST_PATH`としてCriticを呼ばず、応答契約失敗だけをfresh proposal-only Criticへ一回委譲してHost `RootPlanningValidator`へ戻す。CLIはattempt/refinement metadataをboundedに投影する。これはローカルcomposition evidenceであり、live Planner correction、AR1、Phase 8 LIVE_ACTIVATIONのEvidenceではない。Evidenceは[`planner-shadow-bounded-convergence-20260917.json`](../spec/v2/evidence/planner-shadow-bounded-convergence-20260917.json)。
2026-09-17追加: 内部リポジトリ情報を送らないredacted Host-process L2 Planner probeはProvider応答まで到達したが、strict JSONを返さず`model_output_invalid`となった。これは前回の`transport_unclassified`/reconciliationとは別の応答契約失敗であり、同一要求の再送やGate昇格は行わない。`b3630a2`でCLIのbounded分類を明示化し、`19b7770`でfresh requestの`parent_task_id`を失敗projectionへ保持した。`62a9a81`ではinvalid JSON/invalid proposal/parent不一致/child上限違反をfresh `request_id`へ相関し、response observedの投影も例外属性へ束ねた。`63ecba2`ではblocked external failureから生exception messageを除去し、bounded error/transport metadataだけを残した。Evidenceは[`phase8-planner-redacted-model-output-20260917.json`](../spec/v2/evidence/phase8-planner-redacted-model-output-20260917.json)、[`planner-response-correlation-20260917.json`](../spec/v2/evidence/planner-response-correlation-20260917.json)、[`planner-shadow-bounded-error-projection-20260917.json`](../spec/v2/evidence/planner-shadow-bounded-error-projection-20260917.json)を参照する。
- 2026-09-17追加: `03e798a`ではProviderError transport failure、bounded pool exhaustion、response-contract failureの全てをfresh UUID `request_id`へ相関するprojectionを追加した。raw response/exception messageは保存せず、既存のUNKNOWN/reconciliation・failover・retry authorityは変更していない。これはPhase Aの診断強化であり、fresh L2 Planner proposal、AR1 recovery、Phase 8 LIVE_ACTIVATIONのEvidenceではない。Evidenceは[`planner-transport-request-correlation-20260917.json`](../spec/v2/evidence/planner-transport-request-correlation-20260917.json)を参照する。
- 2026-09-17追加: `73473ef`ではPlanner Shadowの予期しない例外fallbackもraw messageなしの`planner_shadow_failure` projectionへ統一し、外部到達有無を断定できないため`reconciliation_required=true`で閉じる回帰を追加した。これはdiagnostic hardeningであり、再送、failover、Planner成功、AR1、Phase 8 Gate昇格を意味しない。Evidenceは[`planner-shadow-unexpected-failure-redaction-20260917.json`](../spec/v2/evidence/planner-shadow-unexpected-failure-redaction-20260917.json)を参照する。
- 2026-09-17追加: `e66f3e3`ではPlanner応答契約の`invalid_json` / `invalid_proposal`を構造化adapter属性として保持し、例外文言依存の分類を除去した。これはPhase Aのbounded診断整理であり、Provider再送、failover、AR1、Phase 8 Gate昇格を意味しない。Evidenceは[`planner-response-contract-structured-classification-20260917.json`](../spec/v2/evidence/planner-response-contract-structured-classification-20260917.json)を参照する。
- 2026-09-17追加: `7b736b8`では、Provider応答まで到達したPlannerの`invalid_json` / `invalid_proposal`に限り、Hostが別admissionしたL1 planning Criticへ一回だけ補正を委譲する経路をPlanner Shadow CLIへ接続した。Criticは`corrected_proposal`のみを返し、既存の`RootPlanningValidator`が最終判定する。成功Planner、transport/quota/security/UNKNOWN failureはCriticを呼ばず、同一provider/binding/modelの重複poolも拒否する。これはCLI接続を含む契約・ローカル検証Evidenceであり、live Planner recovery、AR1、Phase 8 LIVE_ACTIVATIONではない。Evidenceは[`planner-independent-critic-contract-20260917.json`](../spec/v2/evidence/planner-independent-critic-contract-20260917.json)を参照する。
- 2026-09-17追加: `7000102`では、独立Planner CriticのProvider failureとresponse-contract failureにCritic自身のfresh `request_id`を付与し、Shadowの外側projectionではUUID以外を出さないようにした。これは再送やfailoverではなく診断相関の補強であり、UNKNOWN/reconciliation、AR1、Phase 8 Gate意味論を変更しない。Evidenceは[`planner-critic-failure-correlation-20260917.json`](../spec/v2/evidence/planner-critic-failure-correlation-20260917.json)を参照する。
- 2026-09-17追加: `fafd41b`ではPlanner Criticの既存adapter/schema/error/composition helperをlazy `src.dev_agent.intelligence` exportへ接続した。既存の遅延import方針とproposal-only/Host authority境界を維持し、live Provider requestやGate遷移は行っていない。Evidenceは[`planner-critic-public-boundary-20260917.json`](../spec/v2/evidence/planner-critic-public-boundary-20260917.json)を参照する。
- 2026-09-17追加: `5a778d7`ではPlanner Shadow CLIのCritic failure投影にあった例外束縛漏れを修正し、独立Criticのbounded response-contract failureを`model_output_invalid`として安全に投影する回帰を追加した。fresh request UUID以外のProvider/exception detailは出さず、再送・reconciliation・Gate遷移は行っていない。Evidenceは[`planner-critic-cli-failure-projection-20260917.json`](../spec/v2/evidence/planner-critic-cli-failure-projection-20260917.json)を参照する。
- 2026-09-17追加: fresh Host-process Planner probeでは、qualified L2 Plannerと別admissionのCloudflare L1 Criticを使ったが、Plannerが`provider_decode`/`reconciliation_required=true`で停止した。これはPlanner response-contract failureではないためCriticを呼ばず、同一operationの再送・failover・Gate遷移も行っていない。Evidenceは[`planner-critic-live-probe-20260917.json`](../spec/v2/evidence/planner-critic-live-probe-20260917.json)を参照する。
- 2026-09-17追加: `e84ed8d`ではPlanner Shadow成功結果へ、独立Criticの`configured`/`invoked`、Critic自身のfresh UUID、bounded dispatch auditを投影した。Critic adapterは各correction開始時に前回UUIDをリセットし、successful Plannerは引き続きCriticを呼ばない。raw Provider material、credentials、retry/failover、UNKNOWN/reconciliation、AR1、Phase 8 Gateは変更していない。Evidenceは[`planner-critic-observation-projection-20260917.json`](../spec/v2/evidence/planner-critic-observation-projection-20260917.json)を参照する。
- 2026-09-17追加: freshな`gemini:worker:free-4` / `free-5` L2 Planner probeは、既存bounded UNKNOWN-quota admissionを通過した後、両方とも`provider_unavailable`で終了した。Planner responseは観測されず、別admissionのCriticは呼ばれていない。これはPhase Aの外部availability blockerとして記録し、同一binding再送、prior UNKNOWN replay、L1 downgrade、Gate遷移は行っていない。Evidenceは[`phase8-planner-alternate-provider-unavailable-20260917.json`](../spec/v2/evidence/phase8-planner-alternate-provider-unavailable-20260917.json)を参照する。
- 2026-09-17追加: configured poolの既定`gemini-3.5-flash-lite`が現行L2 qualificationと不一致だったため、明示的にqualifiedな`gemini:worker:free-3`の`gemini-3.6-flash`をPlannerへ、単独のCloudflare L1をCriticへ割り当てた。二つのfresh Host-process request（別modelへの一軸変更を含む）はいずれも`provider_unavailable`で応答前に終了し、Criticは未起動だった。これはR9のconfirmed external availability blockerであり、過去UNKNOWNの再送、同一binding/modelのblind retry、L1 downgrade、Gate昇格は行っていない。Evidenceは[`phase8-planner-fresh-provider-unavailable-20260917.json`](../spec/v2/evidence/phase8-planner-fresh-provider-unavailable-20260917.json)を参照する。
- 2026-09-17追加: `5cb658a`ではPlanner Shadowの失敗経路にも、独立Planner Criticの`configured`/`invoked`、呼出し時のfresh UUID、bounded dispatch auditを投影する正規化を追加した。raw Provider/exception detailは保存せず、transport/quota/security/UNKNOWN failureはCritic補正へ流さない。これは相互チェックの診断観測性であり、live Planner correction、AR1、Phase 8 LIVE_ACTIVATION、retry/failover/replay、Gate遷移のEvidenceではない。Evidenceは[`planner-critic-failure-observation-20260917.json`](../spec/v2/evidence/planner-critic-failure-observation-20260917.json)を参照する。
- 2026-09-17追加: `03491b9`では、`run_shadow`の実Provider failure経路でCriticが未起動のままreconciliationを維持し、限定されたfailure observationだけを投影する回帰を追加した。これはPlanner cross-checkのfailure-path coverageであり、live Planner correction、AR1、Phase 8 LIVE_ACTIVATION、retry/failover/replay、Gate遷移のEvidenceではない。Evidenceは[`planner-critic-failure-observation-20260917.json`](../spec/v2/evidence/planner-critic-failure-observation-20260917.json)を参照する。
2026-09-17追加: `diagnose_model_candidates.py`のsummaryは、Catalog全体件数とruntime admission結果を分離する。現行snapshotは`static_eligible_count=127`、`runtime_unknown_count=7`、最終`eligible_count=0`であり、後者をCatalog空と解釈しない。Evidenceは[`model-candidate-runtime-projection-20260917.json`](../spec/v2/evidence/model-candidate-runtime-projection-20260917.json)を参照する。
...
- 現在: `src/dev_agent/mcp/runtime.py`のtransport-neutral adapterと、`scripts/devfarm_mcp.py`の固定run/root Supervisor bindingを実装。status/artifact_summary/run/resume/review/rework/integrateを既存`CodexSupervisedCommanderRun`へ委譲し、plan proposal系は未接続のままbounded rejectionとする。加えてHuman Proxy/Expert Assist専用のbounded JSON-lines adapterを追加したが、外部Codex MCP endpointのlive E2EとPlanner mutation authorityは未接続。証拠は[`mcp-runtime-adapter-20260914.json`](../spec/v2/evidence/mcp-runtime-adapter-20260914.json)と[`operational-human-assist-self-update-local-20260923.json`](../spec/v2/evidence/operational-human-assist-self-update-local-20260923.json)。
...
- 実稼働中のgeneration 3 Coordinatorがidle中に投入されたdurable fake Taskを次周期で完了し、durable stop後にgeneration 4へ再起動してTask状態を保持した。これは既存Operation loopのG-3 idle→work / G-4 restart実process確認であり、OS常駐や配備後Guardian recoveryの証拠ではない。Evidenceは[`operation-runtime-live-idle-restart-20260917.json`](../spec/v2/evidence/operation-runtime-live-idle-restart-20260917.json)。
