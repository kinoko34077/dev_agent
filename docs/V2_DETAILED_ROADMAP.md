...
2026-09-17追加: 内部リポジトリ情報を送らないredacted Host-process L2 Planner probeはProvider応答まで到達したが、strict JSONを返さず`model_output_invalid`となった。これは前回の`transport_unclassified`/reconciliationとは別の応答契約失敗であり、同一要求の再送やGate昇格は行わない。`b3630a2`でCLIのbounded分類を明示化した。Evidenceは[`phase8-planner-redacted-model-output-20260917.json`](../spec/v2/evidence/phase8-planner-redacted-model-output-20260917.json)を参照する。
