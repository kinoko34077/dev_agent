...
- Phase Aのredacted Host-process Planner probeではProvider応答まで到達したがstrict JSON decodeに失敗し、`model_output_invalid`としてboundedに分類できるよう`PlanningResponseError`とCLI projectionを追加した。`19b7770`では失敗時のfresh `parent_task_id`もprojectionへ残す。これは応答契約・相関診断の改善であり、Planner成功、AR1 recovery、Phase 8 live activationを意味しない。Evidenceは[`phase8-planner-redacted-model-output-20260917.json`](../spec/v2/evidence/phase8-planner-redacted-model-output-20260917.json)。
