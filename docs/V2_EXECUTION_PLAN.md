...
| **Next Development Target**（次に着手する開発） | Handoffのreference-first／rework境界とdevelopment-only Codex Supervisorのbounded one-pass wait/resumeを追加中。Supervisorは既存Commanderをcompositionするだけで、外部WorkerのSTATIC_ONLY境界、自動integrationなし、Compression Service未接続、実Planner/Reviewer adapter未接続を維持する。独立Compression Service接続、simulated-paid runtime E2E、有限multi-cycleはHumanの明示指示待ち。 |
