...
- Process Coordinationの基礎として、既存のCoordination store/mailbox/artifactへWork Address、Resume CapsuleのLIFO割込みstack、Host-owned Egress Manifest、generation-fenced ControlRequest、bounded Guardian action journal/evaluationを追加した。これはTask Schedulerやprocess launcherではなく、D9 real mutation前の準備境界である。Guardianの起動・停止・再起動、drain、revision-pinned runtime、OS service、rollbackは未接続。
