...
このCurrent State sliceのコード基準・検証HEADは `e58e658` である。下記のPhase 6/7履歴 baseline は当時の証跡を保持し、capability fail-fast／binding lane wake修正後の全回帰は `600 passed, 1 skipped`（115.17秒）で確認している。

以下の詳細段落に残る過去のrevision／テスト件数は履歴証跡であり、現行の実装・検証基準は上記 `e58e658` と `600 passed, 1 skipped` である。現在状態の参照先は [`docs/CURRENT_STATE.md`](CURRENT_STATE.md) とする。
...
以下の詳細段落には、以前の監査時点のrevision／件数を含む履歴証跡がある。現行の判断・件数・schemaは、この節の`e58e658`／`600 passed, 1 skipped`と`docs/CURRENT_STATE.md`を正本とする。
