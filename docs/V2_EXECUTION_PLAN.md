...
Cloudflare Workers AIは `spec/v2/evidence/phase6-cloudflare-free-2026-09-09.json` でcanonical live qualification済み。Groqは明示的に再読込した資格情報でHTTP 403、SambaNovaは `SAMBANOVA_API_KEY` を検出してAPIへ到達した後HTTP 429となり、いずれも未 qualification のままである。SambaNovaの標準ライブラリHTTP Adapter、OpenAI互換Chat Completions payload、ToolCall decoder、request/day rate-limit header正規化、canonical qualification入口を追加したが、無償tier・課金状態・Gate昇格は推測しない。SambaNovaの失敗証跡は `spec/v2/evidence/phase6-sambanova-free-2026-09-09.json` に保存している。実ProviderのToolCall／ToolResult／quota観測証跡は各成功artifactでのみ確定し、G6O1の有償Provider外部条件や既存Gate判定は変更していない。ControllerのProvider向けassistant tool-call transcriptとqualification scriptのcanonical往復もcheckpoint対象として固定した。今回のexact-head CIは `v2 tests` run `34348650408` と `v2-core` run `34348650509` がいずれもsuccessである。
