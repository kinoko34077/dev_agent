...
| **Next Development Target**（次に着手する開発） | Handoffのreference-first／rework境界、development-only Codex Supervisorのbounded `advance`／blocking `run_until_intervention`、orphan deadline recovery、compact review packet、durable review decision、明示承認後のHost integration helper、Codex action return境界、固定Worker capability probeまで実装済み。次は段階的な実Free Worker Probe観測と、明示承認を得た極小Supervisor dogfood（成功・REWORK・依存Task）である。外部WorkerのSTATIC_ONLY境界、自動integrationなし、Compression Service未接続、G6O1-SIM/LIVE未検証を維持する。 |
