...
- `b29019a`では、Provider応答が観測済みの`invalid_json` / `invalid_proposal`に限り、明示的に別admissionされたL1 planning Criticへ一回だけ補正を委譲し、修正版を既存`RootPlanningValidator`へ戻すbounded compositionを追加した。成功したPlannerはCriticを呼ばず、transport/quota/security/UNKNOWNはCriticへ流さない。Planner/Criticの同一provider/binding/model identityは拒否し、CLIのCritic pool指定も非secret binding metadataに限定する。これは契約・ローカル検証であり、live Planner recovery、AR1、Phase 8 LIVE_ACTIVATIONを意味しない。Evidenceは[`planner-independent-critic-contract-20260917.json`](../spec/v2/evidence/planner-independent-critic-contract-20260917.json)。
