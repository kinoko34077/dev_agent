...
| **Next Development Target**（次に着手する開発） | Handoff Control semantics（source/authority/continuation/comparison/exclusion/output contract）を実装済みにした。独立Compression Service接続、実Planner/Reviewer adapter、simulated-paid runtime E2E、有限multi-cycleはHumanの明示指示待ちであり、現行sliceから自動着手しない。Group Dの残件、MCP Adapter、OS Sandbox、Evidence routing接続、Workflow Promotionも別Gate |
