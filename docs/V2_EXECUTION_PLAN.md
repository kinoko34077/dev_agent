...
実装基準 `9cfac134a1e34c2228acde75ba5b49c65111a57a` では、Groq／Mistral／OpenRouter／SambaNovaのOpenAI互換HTTP共通層、Cloudflareの推定Neuron telemetry、Groq `/v1/models` 診断、fail-closedなlive qualification入口、開発専用Worker Farmのmanifest／result／worktree境界、RoutingSnapshotによるresource/quota観測の一括読出しを実装している。通常のProvider経路は引き続き Controller -> ProviderDispatcher -> ProviderRegistry -> concrete Provider を正本とし、Controllerのdirect経路はcompatibility/legacyに限定する。direct経路のintent／audit／replayは`LegacyDirectProviderJournal`、Routerは`ResourceReadView`、Provider healthは`ProviderHealthStore`へ分離した。Worker Farmは正式runtimeのScheduler／AgentBackend／Phase 7 Multi-Agentではなく、組み込み保護領域を含む契約検証済みの開発補助境界である。R2で旧v1 runtime・logs・memory・prompts・root設定・rootテストをv2 checkoutから除去し、正本を`legacy/v1-final`へ隔離した。

Cloudflare Workers AIは `spec/v2/evidence/phase6-cloudflare-free-2026-09-09.json` でcanonical live qualification済み。OpenRouter Freeも `spec/v2/evidence/phase6-openrouter-free-2026-09-09.json` で同経路のToolCall往復、durable audit、budget reconciliationまで確認済みだが、quota残量は未報告である。Groqは明示的に再読込した資格情報でmodels endpoint HTTP 403（診断証跡 `spec/v2/evidence/groq-models-2026-09-09.json`）、SambaNovaは `SAMBANOVA_API_KEY` を検出して `/v1/models` までHTTP 200で到達した後、推論がHTTP 429/402となり、いずれも未 qualification のままである。Mistralは共通OpenAI互換HTTP Adapterと資格化入口を追加したが、現在の環境に `MISTRAL_API_KEY` がないためlive未実証である。SambaNovaの無償tier・課金状態・Gate昇格は推測せず、free-providerのno-charge qualification入口から意図的に除外している。実ProviderのToolCall／ToolResult／quota観測証跡は各成功artifactでのみ確定し、G6O1の有償Provider外部条件や既存Gate判定は変更していない。ControllerのProvider向けassistant tool-call transcriptとqualification scriptのcanonical往復もcheckpoint対象として固定した。直近のローカル全回帰は `317 passed, 1 skipped` で、実装baselineに対するexact-head CIは `v2-core` run `34360260517` と `v2 tests` run `34360260443` がsuccessである。
