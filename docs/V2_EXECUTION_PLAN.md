...
実装基準 `60bf924dfe22eaf771bbb15f982f25001e607c1c` では、Groq／Cloudflare Workers AI／SambaNovaの標準ライブラリHTTP Adapter、Groq／SambaNova rate-limit headerの正規化quota observation、fail-closedなlive qualification入口、開発専用Worker Farmのmanifest／result／worktree境界、RoutingSnapshotによるresource/quota観測の一括読出しを実装している。通常のProvider経路は引き続き Controller -> ProviderDispatcher -> ProviderRegistry -> concrete Provider を正本とし、Controllerのdirect経路はcompatibility/legacyに限定する。Worker Farmは正式runtimeのScheduler／AgentBackend／Phase 7 Multi-Agentではなく、組み込み保護領域を含む契約検証済みの開発補助境界である。R2で旧v1 runtime・logs・memory・prompts・root設定・rootテストをv2 checkoutから除去し、正本を`legacy/v1-final`へ隔離した。

Cloudflare Workers AIは `spec/v2/evidence/phase6-cloudflare-free-2026-09-09.json` でcanonical live qualification済み。Groqは明示的に再読込した資格情報でHTTP 403、SambaNovaは `SAMBANOVA_API_KEY` を検出して `/v1/models` までHTTP 200で到達した後、推論がHTTP 429/402となり、いずれも未 qualification のままである。SambaNovaの標準ライブラリHTTP Adapter、OpenAI互換Chat Completions payload、ToolCall decoder、request/day rate-limit header正規化を追加したが、無償tier・課金状態・Gate昇格は推測しない。SambaNovaはfree-providerのno-charge qualification入口から意図的に除外し、失敗証跡を `spec/v2/evidence/phase6-sambanova-free-2026-09-09.json` と `spec/v2/evidence/phase6-sambanova-gpt-oss-120b-2026-09-09.json` に保存している。実ProviderのToolCall／ToolResult／quota観測証跡は各成功artifactでのみ確定し、G6O1の有償Provider外部条件や既存Gate判定は変更していない。ControllerのProvider向けassistant tool-call transcriptとqualification scriptのcanonical往復もcheckpoint対象として固定した。実装commit `60bf924dfe22eaf771bbb15f982f25001e607c1c` のexact-head CIは `v2 tests` run `34348650408` と `v2-core` run `34348650509` がいずれもsuccessである。
