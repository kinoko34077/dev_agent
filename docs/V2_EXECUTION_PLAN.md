...
進行判定: Stage F foundation と Stage G operational を別々に自動判定する。G6O2〜G6O6は各責務の証拠でVERIFIED、G6O1は実paid Providerとdeployment-owned設定という外部条件でBLOCKED_EXTERNALのまま維持する。Phase 7CのEvaluator foundationは、Gateを昇格させず、後続実行統合の準備として限定的に進める。exact-head CIは `spec/v2/GATE_STATUS.json` の外部証跡方針に従う。

### 2026-09-10 の現在状態

実装基準 `8b638a9` では、Groq／Mistral／OpenRouter／SambaNovaのOpenAI互換HTTP共通層、Cloudflareの推定Neuron telemetry、Groq `/v1/models` 診断、fail-closedなlive qualification入口、開発専用Worker Farmのmanifest／result／worktree境界、RoutingSnapshotによるresource/quota観測の一括読出し、ResourceCatalog / Observation / Quota store、Phase 7Cの決定的Evaluatorとdurable evidence記録を実装している。通常のProvider経路は引き続き Controller -> ProviderDispatcher -> ProviderRegistry -> concrete Provider を正本とし、Controllerのdirect経路はcompatibility/legacyに限定する。direct経路のintent／audit／replayは`LegacyDirectProviderJournal`、Routerは`ResourceReadView`、Provider healthは`ProviderHealthStore`へ分離した。Worker Farmは正式runtimeのScheduler／AgentBackend／Phase 7 Multi-Agentではなく、組み込み保護領域を含む契約検証済みの開発補助境界である。R2で旧v1 runtime・logs・memory・prompts・root設定・rootテストをv2 checkoutから除去し、正本を`legacy/v1-final`へ隔離した。

Cloudflare Workers AIは `spec/v2/evidence/phase6-cloudflare-free-2026-09-09.json` でcanonical live qualification済み。OpenRouter Freeも `spec/v2/evidence/phase6-openrouter-free-2026-09-09.json` で同経路のToolCall往復、durable audit、budget reconciliationまで確認済みだが、quota残量は未報告である。Groqは明示的に再読込した資格情報でmodels endpoint HTTP 403（診断証跡 `spec/v2/evidence/groq-models-2026-09-09.json`）、SambaNovaは `SAMBANOVA_API_KEY` を検出して `/v1/models` までHTTP 200で到達した後、推論がHTTP 429/402となり、いずれも未 qualification のままである。Mistralは `MISTRAL_API_KEY` 読込み後のlive attemptがHTTP 429となり、証跡 `spec/v2/evidence/phase6-mistral-2026-09-09.json` に記録している。SambaNovaの無償tier・課金状態・Gate昇格は推測せず、free-providerのno-charge qualification入口から意図的に除外している。Cloudflare / OpenRouterのDevFarm Worker試行はAPI到達後にpatch validationで拒否され、host verified testや公式branch統合はまだない。実ProviderのToolCall／ToolResult／quota観測証跡は各成功artifactでのみ確定し、G6O1の有償Provider外部条件や既存Gate判定は変更していない。ControllerのProvider向けassistant tool-call transcriptとqualification scriptのcanonical往復もcheckpoint対象として固定した。直近のローカル全回帰は `349 passed, 1 skipped` で、最後に外部確認したexact-head CIは`30a1cd3`の `v2-core` run `34374695007` と `v2 tests` run `34374695021` がsuccessである。
