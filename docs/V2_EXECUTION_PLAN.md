...
進行表示は、歴史的なPhase 6 operational gateと混同しないよう、`spec/v2/GATE_STATUS.json`の`development_tracks`を併用する。現在は`D9_DOGFOOD=NOT_READY`だが、その一件限定の実行前提は`execution_readiness=READY_FOR_TRIAL`、`D9_PRODUCTION_DEPLOYMENT=DEFERRED_NOT_READY`、`PHASE8_PREPARATION=PREPARATION_ONLY`である。`READY_FOR_TRIAL`はGate完了やapproval消費を意味せず、Task Scheduler保留はDogfood試行準備やPhase 8準備の完了扱いでも、Production Deploymentの検証扱いでもない。
