...
| **External Blockers**（コード変更では閉じられない外部条件） | G6O1（`spec/v2/G6O1_DEFERRED.md`により未検証のまま凍結・現行roadmapは非ブロッキング。実paid Providerのworst-case課金実証＋deployment-owned budget設定は将来再開時の外部条件）、OS_SANDBOXED（filesystem/network/process/resource isolationの実証）、GitHub branch protectionのrequired status checks未設定（`gh`で現状を確認済み。設定は外部Gate） |
