...
| **Next Development Target**（次に着手する開発） | 日常Supervisor導線と実Free WorkerのHost Verification／durable review／deterministic integration／REWORK／依存baseline更新の既存証拠を維持する。次は、今回追加したsession identity契約を前提に、artifact identity/reference extractionを狭く検証し、その後にrestart/reconciliationで外部session discoveryが無い場合のfail-closed境界を設計する。実Gemini WorkerのGroup D候補は2 bounded attemptがWorker境界例外で失敗しており、成功dogfoodへは未到達。外部WorkerのSTATIC_ONLY境界、自動push/mergeなし、Compression Service未接続、G6O1-SIM/LIVE未検証、OS-level sandbox未提供を維持する。 |
