...
| **Next Development Target**（次に着手する開発） | 日常Supervisor導線（短い開始指示、unfinished Plan resume、blocking `run`、ReviewPacket、CLI review/rework/integrate、Host deterministic integration）と、実Gemini L1 WorkerによるGroup D usage回帰Taskを確認済み。次はこの証拠を維持したまま、Group Dのsession identity／artifact extraction／restart/reconciliationを狭いWorker候補Taskへ分割して投入する。外部WorkerのSTATIC_ONLY境界、自動push/mergeなし、Compression Service未接続、G6O1-SIM/LIVE未検証、OS-level sandbox未提供を維持する。 |
