...
8. AgentBackendを別境界で追加する。thin contract、typed admission付きdispatcher、ModelProvider／AgentBackend execution-target seam、CodexExecBackend、明示discovery authority境界まで完了。MCPは既存Supervisorへ委譲するtransport-neutral adapterまで実装し、wire transportとplanner authorityは未接続。
