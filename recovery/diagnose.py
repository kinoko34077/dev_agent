...
try:
    ...
    from .validate_queue import validate_scheduler_queue
...
def run_diagnostics(root: str | Path, *, test_report: str | Path | None = None, artifact_root: str | Path | None = None, resource_ledger: str | Path | None = None, scheduler_queue: str | Path | None = None) -> list[Diagnostic]:
    ...
    checks = (
        ...
        ("configuration", (path / "config" / "v2.yaml").is_file(), "config/v2.yaml exists; contents not read"),
    ...
    if scheduler_queue is not None:
        ok, detail = validate_scheduler_queue(scheduler_queue)
        diagnostics.append(Diagnostic("scheduler_queue", ok, detail))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--scheduler-queue", type=Path, help="validate the Phase 6 scheduler queue")
    parser.add_argument("--json", action="store_true", dest="as_json")
    args = parser.parse_args(argv)
    diagnostics = run_diagnostics(args.root, test_report=args.test_report, artifact_root=args.artifact_root, resource_ledger=args.resource_ledger, scheduler_queue=args.scheduler_queue)
    if args.as_json:
