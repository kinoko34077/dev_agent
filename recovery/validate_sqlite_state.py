...
REQUIRED_TABLES = frozenset({"tasks", "steps", "tool_results", "events", "checkpoints", "idempotency", "approvals", "approval_consumptions", "effect_intents", "effect_reconciliations", "provider_dispatch_audits", "task_controls", "human_requests", "discord_bindings", "discord_ingress", "discord_deliveries", "discord_scopes", "discord_conversation_messages", "schema_meta"})
...
def validate_sqlite_state(path: str | Path) -> tuple[bool, str]:
    ...
    try:
        ...
        if version_row is None or version_row[0] != "10":
