...
def validate_scheduler_queue(path: str | Path) -> tuple[bool, str]:
    ...
    try:
        with sqlite3.connect(f"file:{database.resolve()}?mode=ro", uri=True) as connection:
            ...
            required_columns = {"task_id", "priority", "run_at", "state", "lease_owner", "lease_until", "lease_token", "state_version", "attempts", "max_attempts"}
            ...
            for row in connection.execute("SELECT task_id, run_at, state, lease_owner, lease_until, lease_token, state_version, attempts, max_attempts FROM queue_items"):
                task_id, run_at, state, owner, lease_until, token, state_version, attempts, max_attempts = row
