...
- 公開入口: `HttpCompressionService.from_configured_credentials()`（明示env優先、lazyな固定Credential Manager fallback）、operator smoke用`from_credential_manager()`、`HttpCompressionService.compress(text, profile="semantic-dense-v1")`、`compress_handoff_payload(...)`。
- 入力/出力: 明示された`https://api.kinotch.workers.dev/v1/compress`へ`text`と固定`profile`だけを送信し、compressed text、digest、文字数、Prompt version、model、warningsを検証して返す。認証はfactory呼出時に明示envまたは固定Credential Managerエントリからlazyに読み、import時I/Oやredirect bearer forwardingは行わない。通常の`OneCycleDevelopmentLoop`は設定済みcredentialがある場合だけfactoryを遅延compositionし、tokenなし・設定不備は最適化用途の原文fallbackへ閉じる。呼出側は原文reference、原文digest、Compression provenanceをHandoffへ保持する。
...
- 公開入口: `create(...)`、`status()`、`advance(...)`、`run_until_intervention(...)`、`rework_handoff(...)`、`reassign(...)`、`record_review_decision(...)`、`integrate_approved_worker(...)`、互換用`approve_integration(...)`、`python scripts/devfarm_supervisor.py status|resume|run|review|rework|integrate`。後三者のCLIは既存APIへの薄いoperator adapterである。
