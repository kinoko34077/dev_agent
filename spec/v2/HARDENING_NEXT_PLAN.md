...
進捗: 必須テーブル・schema version・task payload・approval・effect intent・provider dispatch audit に加え、ResourceLedger の schema v6、quota observation、Budget intent binding、orphan Step / checkpoint、壊れた ToolResult、未知の intent status を独立 validator で検出する。Gate A のローカル検査条件は達成。
