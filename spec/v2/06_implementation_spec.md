...
| MCP adapter | `src/dev_agent/mcp/`、`scripts/devfarm_mcp.py` | boundedなtransport-neutral request/result adapter。選択したSupervisor操作を既存authorityへ委譲し、wire transport・Planner authority・独自Scheduler/Retry/Budgetは所有しない |
...
- `devfarm` は production scheduler/state/authority と独立した development-only 層で、既存 WorkerRunner/ProviderFactory 等の公開境界を composition できるが、公式 branch を自動変更しない。`scripts/devfarm_supervisor.py`の`CodexSupervisedCommanderRun`は、既存Commanderのdispatch／collect／verify／reassign／Git integrationをbounded passまたは`run_until_intervention`のblocking運用としてまとめ、optionalなPlan supervisor projectionへwait/cadence/wake/metricsだけを保存する。`review`／`rework`／`integrate` CLIは既存APIへの薄いoperator adapterであり、新Scheduler、background loop、review/integration engine、auto-integrationは追加しない。
- `handoff` は role、instruction、constraints、typed Directive、payload、referenceの型とrendererだけを提供する。Planner/Executor/Reviewer orchestration、Budget、Scheduler、Task lifecycleを所有しない。Controlはpayload compressionへ渡さず、G6O1-SIM/LIVEのbilling identityも別のResource authorityで管理する。Compressionを利用する場合も呼出側の明示compositionに限る。
- `compression` は固定`semantic-dense-v1` profileのHTTP transport、response integrity、payload-only provenance、機械的 retention warningを提供する。`HttpCompressionService.from_configured_credentials()`が明示envを優先し、なければfactory呼出時だけ固定Credential Managerエントリをlazyに読み、3,000 Unicode code points超のPayloadだけを送る。通常の`OneCycleDevelopmentLoop`は設定済みcredentialがある場合だけfactoryを遅延compositionし、import時secret/network I/Oを行わない。構造上限1,000,000とprovider-safe context limit 200,000を分離し、後者超過は送信前にconfiguration failureへ閉じる。Compression Service本体、任意system prompt、Provider routing、Budget authorityを所有しない。失敗はbounded categoryへ正規化し、最適化用途では原文へfallbackできる。
- `mcp` はbounded request/result、redaction、approval要求、timeout/UNKNOWN結果を検査して、callerが提供する既存handlerへ委譲する。`scripts/devfarm_mcp.py`は既存`CodexSupervisedCommanderRun`へ固定run/rootで束ねるdevelopment adapterであり、MCP側へScheduler、Retry、Budget、Approval、Reconciliation、Integration authorityを複製しない。wire transportとplanner proposal/apply authorityは未接続である。
