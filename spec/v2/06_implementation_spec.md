...
- `compression` は固定`semantic-dense-v1` profileのHTTP transport、response integrity、payload-only provenance、機械的 retention warningを提供する。`HttpCompressionService.from_environment()`が固定endpointと`COMPRESSION_API_TOKEN`を明示的に読み、3,000 Unicode code points超のPayloadだけを送る。Compression Service本体、任意system prompt、Provider routing、Budget authorityを所有しない。失敗はbounded categoryへ正規化し、最適化用途では原文へfallbackできる。
