...
- `devfarm` は production scheduler/state/authority と独立した development-only 層で、既存 WorkerRunner/ProviderFactory 等の公開境界を composition できるが、公式 branch を自動変更しない。`scripts/devfarm_supervisor.py`の`CodexSupervisedCommanderRun`は、既存Commanderのdispatch／collect／verify／reassign／Git integrationをboundedな一回のpassとしてまとめ、optionalなPlan supervisor projectionへwait/cadence/wake/metricsだけを保存する。新Scheduler、background loop、auto-integrationは追加しない。
