...
def test_prepare_drain_persists_checkpoint_pauses_claims_and_marks_restart_ready(tmp_path):
    service, agent = _ready_service(tmp_path)
    try:
...
def test_unknown_external_effect_stays_draining_and_never_becomes_restart_ready(tmp_path):
    service, agent = _ready_service(tmp_path)
    try:
        result = service.prepare_drain(
            agent,
...
def test_drain_can_be_completed_after_unknown_effect_is_reconciled(tmp_path):
    service, agent = _ready_service(tmp_path)
    try:
...
def test_drain_rejects_unsupported_external_effect_state(tmp_path):
    service, agent = _ready_service(tmp_path)
    try:
