...
def test_sqlite_store_upgrades_a_v1_schema_through_ordered_migrations(tmp_path):
    ...
    with SQLiteStateStore(database) as store:
        assert store.connection.execute("SELECT value FROM schema_meta WHERE key = 'schema_version'").fetchone()[0] == "11"
        ...
        assert store.connection.execute("SELECT 1 FROM discord_conversation_messages").fetchone() is None
        ...
        assert store.connection.execute("SELECT 1 FROM task_controls").fetchone() is None
