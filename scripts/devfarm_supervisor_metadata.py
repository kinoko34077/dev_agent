"""Durable Supervisor heartbeat and run metadata contracts.

This boundary owns Supervisor status, wake-event, cadence, and metric metadata
only. Review payload normalization lives in ``devfarm_review_protocol`` so a
review contract does not depend on the Supervisor metadata implementation.
"""
...
def normalize_supervisor_metadata(value: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    for item in wake_events[-WAKE_EVENT_LIMIT:]:
        ...
        if FORBIDDEN_EVENT_KEYS.intersection(item):
